...
| `W2` | `backend/.dockerignore` não exclui `world.json` / `items.json` / `ITEM DUMP.json`. O `Dockerfile` faz `COPY . .`, então um build feito na máquina de dev embute ~60 MB de dado estático não-licenciado na imagem, que pode ir para um registry. Fora do escopo da task 01 (versionamento), registrado ao ignorá-los no Git. | **Não se aplica mais (verificado na task 3.6/17).** Os três dumps vivem na raiz do monorepo, fora de `backend/` — que é o contexto de build do `docker build .` documentado (`docker build -t profitpro-backend .`, rodado de dentro de `backend/`). `COPY . .` nunca os alcança, independente do `.dockerignore`. A higiene real que faltava era outra: `runtime-logs/`/`.uv-cache/`/`.cache/`/`*.log` locais, corrigida nesta mesma task |
...
| `W4` | Ranking: a coluna "Retorno" de `frontend/src/opportunities/production-pages.tsx` ficava sempre em `—`. O `recipe_ranking.ingredients` (JSONB) grava `expected_return_quantity: "0"` (neutro); o lucro da linha muda com o slider de retorno (projeção), a coluna não. **Resolvido na task 22**: a coluna separada saiu; a coluna "Receita" mostra o retorno por ingrediente só quando positivo, e o valor exato fica no "Analisar". | Task 22 (resolvido) |
...
| `W6` | A natureza aproximada do ranking (`price_model="neutral_ranking"`, `return_rate` como aproximação linear) não era comunicada na UI. O lucro na lista ≠ o lucro em "Analisar" (`POST /craft/simulate`, exato), sem aviso. **Resolvido na task 22**: o componente `RankingCoverage` traz o aviso "Valores da lista são estimativa — abra 'Analisar' para o cálculo exato". | Task 22 (resolvido) |
| `W7` | `production-pages.tsx` oferecia um controle de frescor até 24 h, mas `max_age_hours` na leitura do ranking só **aperta** a janela de 6 h do rebuild — valores acima de 6 h não tinham efeito. **Resolvido na task 22**: opções capadas em `1/2/6`. | Task 22 (resolvido) |
