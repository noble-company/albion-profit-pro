...
func (op operationAuctionGetOffersResponse) Process(state *albionState) {
	...
	for _, v := range op.MarketOrders {
		...
		if err2 != nil {
			// PATCH LOCAL (Albion Profit Pro): uma entrada que nao e JSON valido nao
			// pode derrubar o client inteiro. O handler irmao
			// (operation_auction_get_requests.go) ja trata o mesmo erro com Errorf.
			log.Errorf("Ignorando entrada de market order que nao e JSON valido: %v -- conteudo: %.160q", err2, v)
			continue
		}
