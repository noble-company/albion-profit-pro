...
|---|---|---|
| `albiondata-client/` | **Fase 2 complete; stabilization pending** (2026-08-23) | Authenticated fork validated with the real game. Fase 2.5 fixes release updater, realm, retry and concurrency before distribution. Market data uses `-i`; never run `gofmt -w` because upstream is CRLF. |
| `backend/` | **Fase 1 + 1.5 complete; stabilization pending** | FastAPI/Celery/PostgreSQL/Redis with 131 tests. Fase 2.5 fixes integrity, failure semantics, realm, seed and operation before frontend work. |
| `frontend/` | **Not created — Fase 3 specified but blocked** | 0/19 tasks. Do not start until the Fase 2.5 gate passes; specs in `docs/tasks/frontend/README.md`. |
| `docs/` | **Fase 2.5 in progress, 1/14 tasks** | Current priority and checklist: `docs/tasks/estabilizacao/README.md`. Read documentation first. |
...
5. Read `docs/05-revisao-fases-0-a-2.md` before touching current code; it is the audit that blocks Fase 3.
6. Implementation specs live in `docs/tasks/{backend,client,estabilizacao,frontend}/`; each README has the dependency graph and authoritative status checklist. Current priority is `estabilizacao`, one task at a time in dependency order.
...
| Market realm | Realm (`west`/`east`/`europe`) must become part of wire identity, facts, cache and reads in Fase 2.5 task 03; until then the system is single-realm-only and must not be distributed broadly. |

## Commands

Backend (`backend/`) is fully implemented (Fase 1 + Fase 1.5 complete) — these commands work today. `frontend/` still doesn't exist.
