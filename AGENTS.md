...
| `backend/` | **Fase 1 + 1.5 complete; stabilization pending** | FastAPI/Celery/PostgreSQL/Redis with 229 tests. Realm, rollups, ingest validation, partial-book semantics, static seed and isolated Celery operation are enforced. |
| `frontend/` | **Not created — Fase 3 is next** | 0/19 tasks. Fase 2.5 is complete; specs in `docs/tasks/frontend/README.md`. |
| `docs/` | **Fase 2.5 complete, 14/14 tasks** | Integrated Windows/game/Swarm validation is deferred until the frontend is complete. Read documentation first. |
...
| Market realm | Realm (`west`/`east`/`europe`) is mandatory in authenticated ingest, facts, cache/pub-sub and reads since Fase 2.5 task 03. Legacy facts were confirmed as West. |
