...
| `frontend/` | **Fase 3.5 complete** | 200 unit tests (46 files) + Playwright E2E against the real stack. Market Flip, Refino, Craft, Calculadora, Preços, Tokens on shadcn/ui + tokens + TanStack Query. |
| `docs/` | **Fase 2.5 complete, 14/14 tasks** | Integrated Windows/game/Swarm validation (task 19) is deferred to the phase gate. Read documentation first. |
...
5. Read `docs/05-revisao-fases-0-a-2.md` (audit of Fases 0–2) and `docs/12-revisao-fase-3.md` (audit of Fase 3 + the outcome of every finding) before touching current code.
6. Implementation specs live in `docs/tasks/{backend,client,estabilizacao,frontend,refatoracao}/`; each README has the dependency graph and authoritative status checklist. Fase 3.5 (`tasks/refatoracao/`) is 28/29, only task 10 (market anti-fraud) left; the remaining work is the task 19 in-game gate. One task at a time in dependency order.
