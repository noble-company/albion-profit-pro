...
| `backend/` | **Fases 1, 1.5, 2.5, the Fase 3.5 backend block and the Fase 4 backend complete; A02, A06 and A07 complete** | FastAPI/Celery/PostgreSQL/Redis, 485 tests (484 passing, 1 skipped), live-client validation. Serves *data* to the browser-side scanner: whole static catalog (`GET /catalog/recipes`), top-of-book snapshot fed by our client **and** the public Albion Data Project API (`GET /prices/snapshot`), sales volume (`GET /prices/sales`); the exact order-book analysis stays in `POST /craft/simulate`. The materialized ranking was retired (4/15). English-only HTTP contract, no price pub/sub. |
| `frontend/` | **Fase 4 (scanner) complete, 38/38; A01–A03 and A05–A07 complete** | 594 unit tests (83 files) + Playwright E2E. Refino, Craft, Comida & Poções, Calculadora and Meus Crafts compute in the browser (`src/scanner/engine.ts`, Web Worker for Craft) over the whole catalog; plus Market Flip, Preços, Painel do Destino, Tokens. Architecture in `docs/15-arquitetura-do-scanner.md`. |
| `docs/` | **Fase 2.5 complete, 14/14 tasks; Fase 4 complete, 38/38; Fase 3.6 superseded** | The Fase 3.6 tasks that still applied (05, 06, 08, 09, 10, 13, 14, 15, 17) and the task 19 integrated in-game gate (2026-09-23) are all done. Only backlog left: task 11 ("Retry real e erro visível", deliberately out of the 3.6 closure scope) and publishing a new static-dataset manifest to actually ship the W11 recipe fix (task 17) to production. Read documentation first. |
...
6. Implementation specs live in `docs/tasks/{backend,client,estabilizacao,frontend,refatoracao,correcoes,scanner}/`; each README has the dependency graph and authoritative status checklist. Fase 3.5 (`tasks/refatoracao/`) is 28/29, only task 10 (market anti-fraud) left. Fase 4 (`tasks/scanner/`) is complete, 38/38, and superseded Fase 3.6. The Fase 3.6 tasks that still applied (`tasks/correcoes/` 05, 06, 08, 09, 10, 13, 14, 15, 17) and the `tasks/frontend/19-build-validacao.md` in-game gate (2026-09-23) are all done. Remaining backlog: task 11 (`tasks/correcoes/11-retry-real-e-erro-visivel.md`, deliberately left out of the 3.6 closure), and publishing a new static-dataset manifest so the task 17 recipe fix (`mount`/`furnitureitem`, +307 recipes) actually reaches production seeding. One task at a time in dependency order.
...
- **The HTTP contract is English-only** (task 3.5/07, `B09`; swept clean in task 3.6/06, `E07`). Every field, query/path parameter, and enum/`const` value on every route uses English names; order-book sides are `sell`/`buy`. Exceptions, matched precisely in `backend/tests/test_api_language.py`: the ingest `*In` schemas (explicit list, Go wire verbatim), and `ApiTokenPublic`/`ClientIdentity`'s `token_sufixo`/`nome`/`ultimo_uso_em` (deferred — `ClientIdentity` is parsed by the Go client). `categoria`/`apenas_craftaveis` on `GET /items/search` still work, `deprecated`, for one cycle.
...
| DB / ORM | PostgreSQL 16 (+ pgvector available, unused for now; `pg_trgm` **is** used, by the item search GIN index — the migration user needs permission for `CREATE EXTENSION IF NOT EXISTS pg_trgm`, or infra pre-creates it, see `docs/06-fontes-de-dados-estaticos.md`) via SQLAlchemy 2.0 async + `asyncpg>=0.31.0` (older asyncpg has no 3.13/3.14 wheels) + Alembic |
...
| Frontend | React 19 + Vite 8 + strict TypeScript + React Router (7.18.2 installed — 8 needs Node 22.22+, this environment is on 22.15; upgrade when the Node floor moves, see `frontend/README.md`) + Tailwind CSS 4 + shadcn/ui + TanStack Query 5; Vitest/RTL/MSW + Playwright |
...
uv run celery -A src.celery_app.celery_app worker -Q ingest -c 4 --loglevel=info  # hot path; Windows: add --pool=solo (W9, prefork breaks with WinError 5)
uv run celery -A src.celery_app.celery_app worker -Q maintenance -c 1 --loglevel=info  # Windows: same
uv run celery -A src.celery_app.celery_app worker -Q quarantine -c 1 --loglevel=info  # Windows: same
...
phase. The task 19 in-game validation (the one human-only piece) is done as of 2026-09-23.
