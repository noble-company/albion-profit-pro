...
| `docs/` | **Fase 2.5 complete, 14/14 tasks** | Integrated Windows/game/Swarm validation (task 19) is deferred to the phase gate. |
...
6. Implementation specs live in `docs/tasks/{backend,client,estabilizacao,frontend,refatoracao}/`. Fase 3.5 (`tasks/refatoracao/`) is 28/29, only task 10 (market anti-fraud) left; the remaining work is the task 19 in-game gate. Implement one task at a time in dependency order.
