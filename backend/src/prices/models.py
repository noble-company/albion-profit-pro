...
class MarketOrder(Base):
    ...
    oferta parada (achado C2). Sem `user_id`/`is_public` de propósito: procedência por
    usuário fica pra tabela de cobertura da task 30. Ver task 27 e
    docs/03-contrato-ingest-real.md secao 5."""

    __tablename__ = "market_order"
    __table_args__ = (
        UniqueConstraint("source_id", name="uq_market_order_source"),
        Index("ix_market_order_book", "item_id", "location_id", "quality_level", "auction_type"),
    )

    id: Mapped[uuid.UUID] = mapped_column(primary_key=True, default=uuid.uuid4)
    # campo "Id" do JSON — id do leilao no jogo, estavel entre varreduras. So globalmente
    # unico enquanto atendermos um servidor so; quando suportarmos multiplos servidores a
    # chave precisa virar (server_id, source_id) — registrar na task da Fase 2 que adicionar
    ...
    location_id: Mapped[str] = mapped_column(String(64), index=True)  # LocationId — ver task 28
...
class MarketHistoryEntry(Base):
    ...
    linha (procedência por usuário é resolvida pela tabela de cobertura da task 30). Ver
    task 26 e docs/03-contrato-ingest-real.md secao 3 (achado N2)."""
...
class MarketScan(Base):
    """Procedência por usuário (task 30): não amarra `user_id` nas tabelas-fato (que são
    ...
    # unique_name do item — o ingest de histórico resolve AlbionId -> unique_name (via `item`,
    # task 28) antes de gravar aqui, pra cobertura ficar num identificador só; o ingest de
...
class MarketHistoryDaily(Base):
    """Rollup diário de `market_history_entry` (task 31): uma linha por (item, local,
...
class MarketHistoryMonthly(Base):
    """Rollup mensal de `market_history_daily` (task 31) — base pra previsão de preço de
